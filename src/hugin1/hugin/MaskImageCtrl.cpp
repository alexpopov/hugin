                         ...
                         const wxString& name)
{
    ...
    Bind(wxEVT_MOUSEWHEEL, &MaskImageCtrl::OnMouseWheel, this);
...
{
    if (!m_previewOnly && wxGetKeyState(WXK_COMMAND))
    {
        if (m_maskEditState == NO_SELECTION || m_maskEditState == POINTS_SELECTED)
        {
            // process mouse wheel event only when there is an active polygon with/without active selection
            // check that mouse is inside the polygon
            wxPoint mpos;
            CalcUnscrolledPosition(mouse.GetPosition().x, mouse.GetPosition().y, &mpos.x, &mpos.y);
            hugin_utils::FDiff2D currentPos = applyRotInv(invtransform(mpos));
            if (m_editingMask.isInside(currentPos))
            {
                // scale polygon around the center point of the polygon
                m_editingMask.scale(mouse.GetWheelRotation() > 0 ? 1.05 : 0.95, m_editingMask.getCenter());
                m_imageMask[m_activeMask] = m_editingMask;
                // update in Panorama class, lets do it by editor panel
                m_editPanel->UpdateMask();
                return;
            };
        }
        else
        {
            ...
            {
                // scale crop rect
                m_cropRect.addBorder((mouse.GetWheelRotation() > 0 ? 0.05 : -0.05) * std::min(m_cropRect.width(), m_cropRect.height()));
                if (!m_cropCircle)
                {
                    // clip to image size, only for rectangular crop
                    // for circular crop let the crop area let go outside of image
                    m_cropRect &= vigra::Rect2D(0, 0, m_realSize.GetWidth(), m_realSize.GetHeight());
                };
                // update in Panorama class, lets do it by editor panel
                m_editPanel->UpdateCrop(true);
                return;
    ...
    // in all other case do normal event processing
    // to allow scrolling with mouse wheel
    mouse.Skip();
}
